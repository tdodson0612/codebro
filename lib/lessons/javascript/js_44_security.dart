...
class Validator {
    ...
    field(name) {
        ...
        const builder = {
            ...
            string: (msg = \`\${name} must be a string\`) => {
                rules.push(v => typeof v === 'string' || msg);
                return builder;
            },
            min: (n, msg = \`\${name} must be at least \${n} chars\`) => {
                rules.push(v => !v || v.length >= n || msg);
                return builder;
            },
            max: (n, msg = \`\${name} must be at most \${n} chars\`) => {
                rules.push(v => !v || v.length <= n || msg);
                return builder;
            },
            pattern: (re, msg = \`\${name} format invalid\`) => {
                rules.push(v => !v || re.test(v) || msg);
                return builder;
            },
            number: (msg = \`\${name} must be a number\`) => {
                rules.push(v => v === undefined || v === null || typeof v === 'number' || msg);
                return builder;
            },
            range: (min, max, msg = \`\${name} must be between \${min} and \${max}\`) => {
...
console.log(\`  Generated CSRF token: \${sessionToken.slice(0, 16)}...\`);
...
try {
    ...
    console.log(\`  ❌ Forged request blocked: \${e.message}\`);
}
...
attempts.forEach((user, i) => {
    try {
        const info = loginLimiter.check(\`login:\${user}\`);
        console.log(\`  Attempt \${i+1}: ✅ Allowed (remaining: \${info.remaining})\`);
    } catch (e) {
        console.log(\`  Attempt \${i+1}: ❌ Blocked — \${e.message}\`);
    }
});
...
headers.forEach(([header, desc]) => {
    console.log(\`  \${header.slice(0, 35).padEnd(36)}: \${desc}\`);
});
